...
class SearchableMixin(object):
    ...
    def search(cls, expression, page, per_page):
        ...
        when = [(item_id, i) for i, item_id in enumerate(ids)]
    ...
    def after_commit(cls, session):
        for obj in filter(lambda obj: isinstance(obj, SearchableMixin), session._changes['add']):
            add_to_index(obj.__tablename__, obj)

        for obj in filter(lambda obj: isinstance(obj, SearchableMixin), session._changes['update']):
            add_to_index(obj.__tablename__, obj)

        for obj in filter(lambda obj: isinstance(obj, SearchableMixin), session._changes['delete']):
